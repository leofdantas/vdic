...
test_that(".msp computes mean square projection", {
  vect <- .create_mock_vectionary()
  result <- .msp("protect care harm", vect$word_projections, vect$dimensions)
...
test_that(".metrics returns all six metrics", {
  ...
  expect_true(all(c("mean", "msp", "sd", "se", "top_10", "top_20") %in% names(result)))
...
test_that("$ operator returns callable functions for valid methods", {
  ...
  msp_fn <- vect$msp
  expect_true(is.function(msp_fn))
...
test_that("vectionary_analyze works with all metric options", {
  ...
  for (metric in c("mean", "msp", "sd", "se", "top_10", "top_20")) {
...
test_that("batch msp matches single-document loop", {
  vect <- .create_mock_vectionary()
  texts <- c("protect care harm", "fair just equal rights")
  batch_result <- vect$msp(texts)
  for (i in seq_along(texts)) {
    single <- vect$msp(texts[i])
    for (dim in vect$dimensions) {
      expect_equal(batch_result[[dim]][i], single[[dim]], tolerance = 1e-10)
    }
  }
